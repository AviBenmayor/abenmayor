...
- **Current answer:** — (D64→D65→D66: childcare REFUSED on aggregator supply (Brooklyn 0.767 [0.545, 1.078]), then the DOHMH roster `gy3q-4tzp` loaded as anchor showed Borough Park was under-coverage (298 → 104 m to nearest childcare) and the re-fit PASSES at 3.556 [2.416, 5.235] on under_18_share itself (t +7.23) — fitted, awaiting the owner's apply. Pharmacy PASSES the gate at 1.585 [1.236, 2.032] but on b(age_18_34_share) = −1.234 (t −3.02), not on b(age_65_plus_share) = +0.264 (t +0.90); read as "pharmacy composition falls where the young are", never "older → more pharmacy demand"; ruling requested to tighten F2 to the primary regressor, which would un-ship it. Neither pharmacy nor childcare home-based care has a full anchor.)
