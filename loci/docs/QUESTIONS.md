...
### O15 — Will shop operators disclose what they pay in exchange for a report on their own address and their block's rent range, and who may ever see the pooled figures?
- **Status:** open
- **Prediction:** —
- **Answered by:** "Stone Street Coffee scored: sales hit as a pre-registered coincidence, floor-area miss #2, and the general no-inferred-floor-area rule (D135)" (E4 · Validation and Artifact) — the miss that motivates it; the pilot itself is not yet ticketed
- **Tier:** P2 — opportunity evidence
- **Why it matters:** Both scored operator misses are inputs, not model form: rent (D130) and floor area (D130, D135). The warehouse holds no shop rents — the listings table holds flats — so economics stays grade D until real rents arrive, and D118's challenger null says a better model will not substitute. Scraping is closed (BizBuySell/BizQuest/LoopNet 403; asking rents are not signed rents), the LL157 FOIL (D126) is unanswered and may return nothing, so operator disclosure is the remaining path. Owner's framing, 2026-09-22: the only durable incentive is give-to-get — share what you pay, get your address on paper plus your block's range — with the contributor protected structurally rather than by promise (ranges never points, nothing shown below a threshold N per block, no neighbour's figure on your own report, deletion recomputes every range). The second half of the question is the one that decides the first: the friend who answered the Stone Street thread asked whether the tool is "a research lever investors could use to buy out leases or businesses," and if operator-contributed figures ever reach investors or landlords, even in aggregate, contribution stops the moment that is noticed. Charter §1 makes the owner the first user under take-it-first, so nothing forces an answer yet — but a site that solicits rents does.
- **How to answer:** Run a 20–30 operator warm-intro pilot before building any platform: offer the existing "on paper" page plus the five pre-registration questions, and count how many disclose a rent. Under ~5 the incentive is too weak for any product to fix; 10+ is a rent panel and a case for the build. Separately, get an owner ruling on the disclosure boundary (operator-side only vs aggregates to brokers/lenders/investors) and on the threshold N, before any collection begins rather than after. Validate whatever arrives against the public baseline — LL157 if the FOIL lands, and the D133 `commercial_listings.py` asking rents if that code ships.
- **Fails if:** operators decline at a rate that leaves no usable panel, or disclosure proves so selective (the aggrieved and the struggling over-represented, as on Glassdoor) that the pooled range cannot be corrected against an independent baseline.
- **Current answer:** Not tested. One operator has volunteered rent directionally ("a little low" against $10,500) and one gave an exact figure unprompted (Fazenda, $15,000/mo, D130) — two of three scored records disclosed something about rent when asked as part of a scored page, which is suggestive and not a sample.
