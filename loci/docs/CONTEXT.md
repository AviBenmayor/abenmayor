...
**The daily-needs bundle** is fifteen categories defined in `src/loci/categories.yaml`, one definition list mirrored into several per-concern files with a drift test, covering OSM, Overture, NAICS and licence mappings, plus a sixteenth, `bathhouse_sauna`, admitted 2026-09-17 through the fail-closed expansion checklist as a **non-filtering signal** (owner ruling, GTM-198: `headline: false`, narrow definition with the clause "day spa out UNLESS bathing is the primary offer"; the pinned definition is in `docs/CATEGORIES.md`), and a seventeenth, `brewery`, admitted 2026-09-22 (owner rulings, D137: its own category, NOT folded into `bar`; WIDE definition covering taprooms, brewpubs-as-breweries AND production-only brewers) also as a **non-filtering signal** (`headline: false`). Both new slugs landed through the checklist only as far as G0 (registry/adapter completeness); G3-G8 need a warehouse fit against real ingested supply and are not passed for either. Five are demoted from headline claims: `clinic` (2026-09-14, D30 precedent), `tailor_repair`, whose measured true-coverage-hole rate is 37.5% [31.1 to 44.4] **and a floor**, `hair_barber` (both D30 precedent), `bathhouse_sauna` (2026-09-17, D124), and `brewery` (2026-09-22, D137). New categories enter only through the fail-closed expansion checklist (GTM-112).
...
| C3 | Two or more POIs at one address trigger a closure check before either is counted. Status 2026-09-21: fixed in code — an unresolved same-category co-located group of <=5 members collapses to one counted POI by default; a larger group (a geocode sink: a terminal food hall at one coordinate) is a documented exception and counts as-is, never routed to a paid closure check. Groups still form only from same-category records at one coordinate cell. The live warehouse re-run that moves `supply_hash` is a separate, peer-announced step (D106/C9/C11) not yet taken. | "any time we have 2 businesses in the same address, we should do a check if one of them closed down" | 2026-09-14 | D94, D132 |
...
Two POIs at one address may be one business and one closure. `poi_is_open` and `analysis.poi_colocation` remove evidenced-closed POIs (2,982, 2.13%). As of the 2026-09-21 owner ruling (D132, GTM-202, closing QUESTIONS D24), an unresolved pair — the evidence cannot split it — collapses to its one positively-open (else lowest-poi_id) survivor by default, `score/supply.canonical_poi_sql`, so it is never counted twice, EXCEPT a group above `SINK_GROUP_SIZE` (5) members, a documented charter exception: a GEOCODE SINK (a terminal food hall published at one coordinate — Penn Station, JFK, Port Authority measured at 36 to 72 members) counts as-is and is not routed to a paid closure check either. Code fix landed in `src/loci/score/supply.py`; the live warehouse re-run (which moves `supply_hash`) is a separate, peer-announced step per D106/C9/C11. DONE 2026-09-22 (D134): the re-run hit a pre-existing, unrelated blocker first — `bathhouse_sauna` (admitted to `categories.py` at D124, 2026-09-17, `ships_as: signal`) had zero ingested POIs and no code path excluded it from the aggregates that `address-gaps` and `supply_ratio.fit_baselines()` compute, so both refused (GTM-209). The owner ruled to fix GTM-209 rather than defer it, checked against `docs/CATEGORY-EXPANSION.md` §4's signal-vs-filter rule ("removing it must leave `gap_score`, `lead_category`, `n_missing` and the missing set byte-identical"), and `model/conveniences.signal_categories()` now backs an exemption at both call sites. The live warehouse re-run then completed end to end: `supply_hash` `ed55301203a4` → `1898163ac8ce` (gated supply 133,356 → 130,536, −2.11%, matching the pre-run preview exactly), `analysis.address_gaps` rebuilt with no error (190,413 lot + 33,934 street addresses read a gap; `bathhouse_sauna` correctly leads nowhere), and the full canonical order (address-demand, pipeline, storefronts, age-fit, supply-ratio, address-access, transit-profile, citibike, address-character, storefront-pipeline, dot-counts, address-legality, revenue, export-webmap) re-ran clean. No prior screen snapshot exists to diff against — `address-gaps` had not completed successfully since D121 (2026-09-17, predating both `bathhouse_sauna` and the collapse fix).
