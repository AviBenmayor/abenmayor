...
| **NYC DOT Bi-Annual Pedestrian Counts** `cqsj-cfgu` (the map view `2de2-6x2h` publishes no columns) | **Validation only, never an input.** 114 screenline points, of which `loc` 1–100 are on-street and 101–114 are bridge midpoints. Manual AM/MD/PM counts, twice a year since 2007. Used to ask whether `transit_entries_400m`, `jobs_400m` and `homes_400m` rank real sidewalk volume; N = 100 is too small for anything but a rank correlation, and the points are DOT-selected toward busy commercial streets, so the correlation is measured on a restricted range | **$0** |
