...
**Last updated:** 2026-09-03 (session 9 — M1 validation corrected via geometry/coverage split, D3 Manhattan sensitivity, validator bug fixed)
...
Phase: Core gap screen shipped (analysis.hex_gaps, `loci gaps`, "The Missing Business" map).
...
spikes now committed under E9. Current focus: per-category walk thresholds from revealed spacing —
the ranked gap list is not trustworthy at a single citywide 800m constant until that lands.
