...
**Last updated:** 2026-09-05 (session 12 — NAICS 2022 adopted as the category anchor (D42): `src/loci/categories.yaml` + vendored Census files + drift test, commit 4436bae. Session 11 in the same day folded in Meltzer & Schuetz and the ZBP validation source (D40). Full suite 94 passed at close.)
...
8. Fitness anchor source for the ~20% true coverage hole (QUESTIONS M7).
9. Clinic re-anchor to licensed urgent care (D30).
10. ACS vehicle-ownership pull per tract (for D7 density-class / mode thresholds). Key is set; test renter_share first (H-L7: owner-occupancy predicts lower retail density) before the vehicle ingest.
11. E9 ignition productionization — not before 1–5.
12. Comps (O7/O8): owner runs the manual export in benchmarks.yaml or approves a browser session;
    ...
    a reference file.
13. Retire hex-only paths from the CLI usage block once the address screen passes the battery; keep
    the tables.
14. ZBP follow-up (D40/M9): rerun `loci zbp-compare` per source to attribute the overcount; then decide whether SLA and NYS DOS anchors need an active-license filter like the DOHMH one in (5). Add ZCTA polygons for a proper POI→ZIP join. Feed `zip_category_establishments` size bands into D9 and establishments-per-resident into O6.
15. Fold `src/loci/zbp_naics.yaml` into `src/loci/categories.yaml` (D42) so there is one NAICS mapping, or have zbp-compare read the anchor's `naics_2017` field; add the drift check.
16. Overture adapter: decide whether `bank_credit_union` belongs under the bank slug (NAICS 522110 excludes credit unions 522130) — see the comment in `categories.yaml`.
~~GTM-12~~ done — see D12.
