...
def test_commercial_poi_categories_is_the_full_retail_registry():
    """Documents the decision the seed left open: every one of the 15
    registered categories is retail/food/personal-service, so all 15 count as
    ...
    updated alongside it -- which is the point of pinning it."""
    from loci.categories import CATEGORIES

    assert al.commercial_poi_categories() == frozenset(CATEGORIES)
    assert len(al.commercial_poi_categories()) == 15
